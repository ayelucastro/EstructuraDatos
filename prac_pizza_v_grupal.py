...
def cocina (lista_cocina):
    """esta funcion hace que la cocina reciba los pedidos"""
    contador = 1
    for item in lista_cocina:
        print(f'Pedido número {contador}: ')
        for i in item:
            print(f'Preparando {i[1]} pizzas de {i[0]}')
        contador += 1
    return None
...
# Estructuras de datos utilizadas
    ...
    #pizza_menos_pedida = []
    lista_pedidos_cocina = []
    #pedido_cocina = tomar_pedido()
# punto a
...
#punto d
        ...
        lista_pedidos_cocina.append(el_pedido)
        cocina (lista_pedidos_cocina)
        tomar_pedidos = input("¿Desea ingresar pedidos? (Si -> enter / x para salir)") #esto es el uso del centinela
        #total_pizzas = total_pizzas + i[1]
        #print('total_pizzas')
